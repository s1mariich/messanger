...
class MessengerApp:
    ...
    def create_chat(self, btn, data):
        ...
        self.master.bind("<Return>", lambda event: self.send_message(frm, message_entry, data))
    ...
    def viewMessages(self, chat, pub):
        ...
        commCount = tk.Label(container, text=str(comments), anchor="nw", bg="#e3e8f5", font=("", 10))
        ...
        likeCount = tk.Label(container, text=str(len(likes)), anchor="nw", bg="#e3e8f5", font=("", 10))
        ...
        btnLike.pack(side=tk.RIGHT, padx=(20, 0))
        if user == self.user:
            container.pack(side="top", pady=(0, 5), anchor="e")
        else:
            container.pack(side="top", pady=(0, 5), anchor="w")

    def comment(self, count, pub):
        rootComm = tk.Tk()
        CommentApp(rootComm, pub, self.user, count)
        rootComm.mainloop()
...
class CommentApp:
    def __init__(self, root, pub, currUser, count):
        ...
        root.bind("<Return>", self.send_comment)
...
class RegisterApp:
    def __init__(self, root):
        ...
        self.button_Register = tk.Button(root, text="Register", font=("", 14), width=20, border=0, bg="#b4c7f7",
                                         command=self.reg)
        self.root.bind("<Return>", self.reg)
    ...
    def reg(self, event=None):
        if self.login_entry.get() != '' and self.pass_entry.get() == '' and self.pass_repeat_entry.get() =='':
            self.pass_entry.focus()
        elif self.login_entry.get() != '' and self.pass_entry.get() != '' and self.pass_repeat_entry.get() =='':
            self.pass_repeat_entry.focus()
        elif self.login_entry.get() != '' and self.pass_entry.get() != '' and self.pass_repeat_entry.get() !='':
...
class AuthorizationApp:
    ...
    def check(self, event=None):
        if self.login_entry.get() != '' and self.pass_entry.get() == '':
            self.pass_entry.focus()
        elif self.pass_entry.get() != '' and self.login_entry.get() != '':
            per = self.login_entry.get()
            data = []
            with open(users, "r") as file:
                logs = list(file.readline().split())
